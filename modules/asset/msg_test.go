	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msg := MsgIssueToken{
				...
				AddrFreezeable:  tt.msg.AddrFreezeable,
				TokenFreezeable: tt.msg.TokenFreezeable,
			}
	...
	tests := []struct {
	...
	}{
		{
			...
			NewMsgIssueToken("123456789012345678901234567890123", "coin", 100000, tAccAddr,
				false, false, false, false),
			ErrorInvalidTokenName(CodeSpaceAsset, "issue token name limited to 32 unicode characters"),
		},
		{
			"testSymbol1",
			NewMsgIssueToken("name", "1a", 100000, tAccAddr,
				false, false, false, false),
			ErrorInvalidTokenSymbol(CodeSpaceAsset, "issue token symbol limited to [a-z][a-z0-9]{1,7}"),
		},
		{
			"testSymbol2",
		...
		},
		{
			...
			NewMsgIssueToken("name", "coin", 9E18+1, tAccAddr,
				false, false, false, false),
			ErrorInvalidTokenSupply(CodeSpaceAsset, "issue token supply amt limited to 90 billion"),
		},
		{
			"testTotalSupply2",
			NewMsgIssueToken("name", "coin", -1, tAccAddr,
				false, false, false, false),
			ErrorInvalidTokenSupply(CodeSpaceAsset, "issue token supply amt should be positive"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestMsgIssueToken_GetSignBytes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			NewMsgIssueToken("test-coin", "coin", 100000, tAccAddr,
				false, false, false, false),
			`{"type":"asset/MsgIssueToken","value":{"AddrFreezeable":false,"Burnable":false,"Mintable":false,"Name":"test-coin","Owner":"cosmos1n9e8krs6dengw6k8ts0xpntyzd27rhj48ve5gd","Symbol":"coin","TokenFreezeable":false,"TotalSupply":"100000"}}`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
